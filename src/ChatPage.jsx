      ...
      <div className="bg-white dark:bg-neutral-800 p-6 rounded-2xl shadow-xl w-[380px]">
        ...
        <div className="flex justify-between mt-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => onSave(project.id, name.trim() || project.name, color)}
...
export default function ChatPage({ user }) {
  ...
  // ✅ ROLE
  const [userRole, setUserRole] = useState(null); // "pending" | "active"
  ...
  }, [currentConv?.messages, loading, showIntroTyping]);
  ...
  const saveProjectEdit = async (projectId, name, color) => {
    const uid = user.uid;
    await updateDoc(doc(db, "users", uid, "projects", projectId), { name, color });

    // 해당 프로젝트에 속한 상담 카드 색 동기화(선택)
    const convSnap = await getDocs(
      query(collection(db, "users", uid, "conversations"), where("projectId", "==", projectId))
    );

    for (let c of convSnap.docs) {
      await updateDoc(doc(db, "users", uid, "conversations", c.id), { color });
    }
  ...
  const deleteProject = async (projectId) => {
    ...
    // 프로젝트에 속한 상담들의 projectId를 null로 변경
    const convSnap = await getDocs(
      query(collection(db, "users", uid, "conversations"), where("projectId", "==", projectId))
    );

    for (let c of convSnap.docs) {
      await updateDoc(doc(db, "users", uid, "conversations", c.id), {
        projectId: null,
        color: null,
      });
    }
  ...
  const addConversation = async () => {
    ...
    // ✅ 첫 메시지는 "타이핑"으로 보여줄 거라, 여기서 미리 저장하지 않음!
    // (tone 선택 후 showIntroTyping → 저장)
  ...
  const deleteConversation = async (convId) => {
    ...
    const msgSnap = await getDocs(collection(db, "users", uid, "conversations", convId, "messages"));
    for (let m of msgSnap.docs) {
      await deleteDoc(doc(db, "users", uid, "conversations", convId, "messages", m.id));
    }
  ...
  const renameConversation = async (convId) => {
    ...
    const title = window.prompt("새로운 상담 제목을 입력해주세요.", conv?.title || "상담");
    if (!title || !title.trim()) return;

    const uid = user.uid;
    await updateDoc(doc(db, "users", uid, "conversations", convId), { title: title.trim() });
  };
  ...
  const saveMessage = async (convId, sender, text) => {
    ...
    await setDoc(doc(db, "users", uid, "conversations", convId, "messages", msgId), {
      sender,
      text: text ?? "",
      createdAt: serverTimestamp(),
  ...
  const requestTitle = async (convLike) => {
    try {
      ...
      const category = projects.find((p) => p.id === convLike.projectId)?.name || "일반";
...
7\\. 실제 피해 사례를 중점으로 한 글`;
    ...
    try {
      const convState = conversations.find((c) => c.id === convId) || currentConv;

      const convForGpt = {
        ...convState,
        messages: [...(convState?.messages || []), { sender: "user", text: trimmed }],
      };
      ...
      const convForTitle = {
        ...convForGpt,
        messages: [...(convForGpt.messages || []), { sender: "bot", text: reply }],
      };

      const newTitle = await requestTitle(convForTitle);
      if (newTitle) {
        const uid = user.uid;
        await updateDoc(doc(db, "users", uid, "conversations", convId), { title: newTitle });
      }
    } finally {
      setLoading(false);
      setInput("");
  ...
  const selectTone = async (toneName) => {
    ...
    // ✅ 이 대화에 메시지가 하나도 없으면 → 인트로 타이핑 시작
    const conv = conversations.find((c) => c.id === currentId);
    const hasAnyMsg = (conv?.messages || []).length > 0;
  ...
  if (userRole !== "active") {
    return (
      <div className="w-screen h-screen flex items-center justify-center text-center px-4 bg-gray-50 dark:bg-black">
        <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl p-8 w-[360px]">
          <h2 className="text-xl font-bold mb-2 dark:text-white">⛔ 접근 제한</h2>
  ...
  /* =========================================================
     ✅ 아래부터는 "active" 유저만 로드되도록 EFFECT를 분리
  ========================================================= */
                  ...
                  return (
                    <div key={pjt.id} className="group relative">
                      ...
                      >
                        ...
                        <span className="font-semibold text-sm truncate">{pjt.name}</span>
                      </button>
